...
/**
 * 处理一次预览请求,返回 { src, mime, label, summary };有错抛 Error。
 * args 支持两种形态:
 *  - { path }:从磁盘读取一个文件;
 *  - { content, mime }:直接用调用方给的内容(base64 或 UTF-8 文本)。
 */
  ...
  return {
    ...
    output: {
      ...
      presentationMeta(args) {
        return buildPreview(args).then(
          (r) => ({ src: r.src, mime: r.mime, label: r.label }),
          () => null,
        )
      },
    },
    async execute(args) {
      try {
        const r = await buildPreview(args)
